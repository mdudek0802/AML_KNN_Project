...
if __name__ == "__main__":
    ...
    regions_train, regions_test = train_test_split(sub_data, train_size=0.7, random_state=0)
    # print(regions_test)
    # print(regions_train)
    # print(regions_test.iloc[:, 0])
    # print(regions_test.iloc[:, 1:])

    classifier = KNeighborsClassifier(K_NEIGHBORS, metric="euclidean")
    ...
    disp = ConfusionMatrixDisplay.from_estimator(
            classifier,
            regions_train.iloc[:, 1:],
            regions_train.iloc[:, 0],
            # display_labels=unique_regions,
            cmap=plt.cm.Blues,
            normalize=None,
        )
